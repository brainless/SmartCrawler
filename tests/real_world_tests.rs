...
async fn full_crawl_pipeline(initial_url: &str) -> Result<(smart_crawler::HtmlNode, UrlStorage), String> {
    ...
    domain_urls.entry(domain.clone()).or_default().insert(initial_url.to_string());
    ...
    if !domain_urls[&domain].contains(&root_url) {
        domain_urls.get_mut(&domain).unwrap().insert(root_url.clone());
            ...
            match process_url(&mut browser, &parser, &mut storage, first_url, true).await {
                Ok(html_source) => {
                    ...
                    for additional_url in additional_urls {
                        ...
                        if domain_urls.get_mut(&domain).unwrap().insert(additional_url.clone()) {
    ...
    let html_tree = storage.get_url_data(initial_url)
...
async fn test_hacker_news_submissions() {
    ...
    match full_crawl_pipeline("https://news.ycombinator.com/").await {
        Ok((tree, storage)) => {
            ...
            let filtered_tree = if let Some(domain_duplicates) = storage.get_domain_duplicates("news.ycombinator.com") {
            ...
            let submissions = filtered_tree.find_by_path("html body center table tbody tr td table tbody tr.athing.submission td.title");
            
            println!("Found {} submission elements after filtering", submissions.len());
            ...
            for (i, submission) in submissions.iter().enumerate() {
                ...
                let status = if content == "[FILTERED DUPLICATE]" { " (FILTERED)" } else { "" };
                println!("Submission {}: '{}'{}", i + 1, content, status);
            }
            
            // Check how many are actual content vs filtered duplicates
            let actual_submissions: Vec<_> = submissions.iter()
                .filter(|s| s.content != "[FILTERED DUPLICATE]")
                .collect();
            
            println!("Actual (non-filtered) submissions: {}", actual_submissions.len());
            println!("Filtered duplicate submissions: {}", submissions.len() - actual_submissions.len());
...
async fn test_mykin_ai_team_member() {
    ...
    match full_crawl_pipeline("https://mykin.ai/company").await {
        Ok((tree, storage)) => {
            // Apply domain-level duplicate filtering if available
            let filtered_tree = if let Some(domain_duplicates) = storage.get_domain_duplicates("mykin.ai") {
                println!("Applying domain-level duplicate filtering...");
                smart_crawler::HtmlParser::filter_domain_duplicates(&tree, domain_duplicates)
            } else {
                println!("No domain duplicates found, using original tree");
            ...
            let team_members = filtered_tree.find_by_path("html.w-mod-js.w-mod-ix body div.page-wrapper main.main-wrapper section.section_team div.padding-global div.container-medium div.team_collection.is-desktop.w-dyn-list div.team_collection-list.w-dyn-items div.w-dyn-item a.team_card.w-inline-block div.team_content h4");
            
            println!("Found {} team member elements after filtering", team_members.len());
            ...
            let kasper_found = team_members.iter()
            ...
            let actual_members: Vec<_> = team_members.iter()
                .filter(|m| m.content != "[FILTERED DUPLICATE]")
                .collect();
            
            println!("Actual (non-filtered) team members: {}", actual_members.len());
            println!("Filtered duplicate team members: {}", team_members.len() - actual_members.len());
